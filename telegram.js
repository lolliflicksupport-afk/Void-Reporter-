...
export function pickProxy(sessionCfg) {
  const p = sessionCfg.proxy;
  if (!p) return null;
...
async function resolveTarget(client, info) {
  if (info.type === "invite") {
    ...
    const inv = await withTimeout(
      client.invoke(new Api.messages.CheckChatInvite({ hash })), 10000, "checkinvite"
    );
    if (inv.chat) return await client.getInputEntity(inv.chat.id);
    throw new Error("invite_unresolved");
  }
  try {
    return await withTimeout(client.getInputEntity(info.value), 10000, "resolve");
  } catch (e) {
...
// ===============================================================
// WORKER вЂ” tries every proxy in the fallback list
// ===============================================================
async function worker(session, job, sem) {
  ...
  try {
    ...
    const proxyList = session.cfg.proxyList?.length
      ? session.cfg.proxyList
      : (session.cfg.proxy ? [session.cfg.proxy] : []);
      ...
      try {
        client = new TelegramClient(
          ...
          CFG.apiId, CFG.apiHash,
          {
            ...
            useWSS: true,
            timeout: 20,
            proxy: pickProxy({ proxy: proxyCfg }),
          }
        );
        await withTimeout(client.connect(), 20000, "connect");
        connected = true;
        session.lastStatus = `ok:proxy${pIdx + 1}`;
        log.tg(`${session.name} connected via proxy ${pIdx + 1} (${proxyCfg.host}:${proxyCfg.port})`);
        break;
      } catch (e) {
        lastErr = e.message || e.constructor.name;
        log.warn(`${session.name} proxy${pIdx + 1} (${proxyCfg.host}:${proxyCfg.port}) failed: ${lastErr}`);
    ...
    if (!connected) {
      try {
        client = new TelegramClient(
          new StringSession(session.cfg.session),
          CFG.apiId, CFG.apiHash,
          { connectionRetries: 1, useWSS: true, timeout: 20 }
        );
        await withTimeout(client.connect(), 20000, "connect_direct");
        connected = true;
        session.lastStatus = "ok:direct";
        log.tg(`${session.name} connected direct (no proxy worked)`);
      } catch (e) {
        lastErr = e.message || e.constructor.name;
        try { await client?.disconnect(); } catch (_) {}
        client = null;
    ...
    if (!connected || !client) {
      await withLock(() => { session.lastStatus = `cf:${lastErr}`; });
      log.err(`${session.name} all proxies + direct failed: ${lastErr}`);
