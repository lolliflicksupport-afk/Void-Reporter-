...
function makeAgent(proxy) {
  ...
  const url = `${proxy.scheme || "socks5"}://${proxy.username ? `${proxy.username}:${proxy.password}@` : ""}${proxy.host}:${proxy.port}`;
  try {
    if (url.startsWith("socks")) return new SocksProxyAgent(url);
    if (url.startsWith("http")) return new HttpsProxyAgent(url);
  } catch (e) {
    log.warn(`agent build failed: ${e.message}`);
    return null;
  }
  return null;
}
...
function clientOptions(sessionCfg) {
  ...
  if (agent) {
    opts.proxy = agent;
    log.tg(`using proxy ${sessionCfg.proxy.host}:${sessionCfg.proxy.port}`);
  } else {
    log.tg(`no proxy вЂ” connecting direct`);
  }
  return opts;
}
