...
function makeAgent(proxy) {
  ...
  const url = `${proxy.scheme || "http"}://${proxy.username ? `${proxy.username}:${proxy.password}@` : ""}${proxy.host}:${proxy.port}`;
  try {
...
function clientOptions(sessionCfg) {
  const opts = {
    connectionRetries: 2,
    timeout: 20,
    useIPV6: false,
  };
  const agent = makeAgent(sessionCfg?.proxy);
