...
function parseProxyUrl(url) {
  ...
  try {
    ...
    const withScheme = /:\/\//.test(trimmed) ? trimmed : `socks5://${trimmed}`;
    const u = new URL(withScheme);
    return {
      scheme: u.protocol.replace(":", "") || "socks5",
      host: u.hostname,
...
function loadSessions() {
  ...
  // priority 2: TG_SESSION_N env vars
  const arr = [];
  let i = 1;
  while (true) {
    ...
    // collect every TG_PROXY_N as fallback for this session
    const proxyList = [];
    let j = 1;
    while (true) {
      const p = (process.env[`TG_PROXY_${j}`] || "").trim();
      if (!p) break;
      const parsed = parseProxyUrl(p);
      if (parsed) proxyList.push(parsed);
      j++;
    }
